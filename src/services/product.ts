...
type ProductUpdateReceivedDTO = Omit<Product, "companyId"> & {
  technicalDetails: InputJsonValue;
  image?: any;
};
...
abstract class ProductService {
  ...
  static async create(companyId: string, product: ProductFormDataReceivedDTO) {
    ...
    if (image) {
      ...
      const productImageExtension = allowedFileExtensions.filter((extension) =>
        product?.image?.originalname?.includes(extension)
      )[0];

      const productImageName = "photo_".concat(
        code.replace("#", ""),
  ...
  static async update(productId: string, newData: ProductUpdateReceivedDTO) {
    ...
    if (newData?.image && foundProduct?.photoUrl != null) {
      const allowedFileExtensions = [".jpg", ".png", ".jpeg"];
      const productImageExtension = allowedFileExtensions.filter((extension) =>
        newData?.image?.originalname?.includes(extension)
      )[0];

      const productImageName = "photo_".concat(
        productToSave.code.replace("#", ""),
        productImageExtension
      );
      ...
      const productImageExtension = allowedFileExtensions.filter((extension) =>
        productToSave?.photoUrl?.includes(extension)
      )[0];
